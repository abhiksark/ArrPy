...
"""
Cython-optimized core Array class implementation for ArrPy v0.2.1.

This module contains the fundamental Array class that provides the foundation
for all array operations in ArrPy, with multicore parallel processing support.
"""
...
cdef class Array:
    """
    ArrPy's fundamental array class implemented in Cython.
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, tuple):
            ...
            if len(key) == len(self._shape):
                ...
                return self._get_single_element(key)
            else:
                # Partial indexing - return sub-array
                return self._get_subarray_tuple(key)
        else:
            # Single index
            if len(self._shape) == 1:
                return self._get_1d_element(key)
            else:
                return self._get_subarray_single(key)
    
    @cython.boundscheck(False)
    @cython.wraparound(False)
    cdef object _get_single_element(self, tuple key):
        """Get single element using tuple indexing."""
        cdef long flat_index = 0
        cdef long multiplier = 1
        cdef long idx
        cdef Py_ssize_t i
    ...
    cdef Array _get_subarray_tuple(self, tuple key):
        """Get subarray using tuple indexing."""
    ...
    cdef object _get_1d_element(self, long key):
        """Get single element from 1D array."""
        # Handle negative indexing
        if key < 0:
            key += len(self._data)
    ...
    cdef Array _get_subarray_single(self, long key):
        """Get subarray using single index."""
        # Handle negative indexing
        if key < 0:
            key += self._shape[0]
        ...
        cdef long sub_size = len(self._data) // self._shape[0]
        cdef long start_idx = key * sub_size
        cdef long end_idx = start_idx + sub_size
    ...
    # Basic arithmetic operations with parallel processing
    def __add__(self, other):
        """Element-wise addition with parallel processing."""
        cdef list other_data = self._check_compatible_shape(other)
        cdef Py_ssize_t n = len(self._data)
        cdef double[::1] result_array = np.empty(n, dtype=np.float64)
        cdef Py_ssize_t i
        
        # Use parallel processing for arrays larger than 1000 elements
        if n > 1000:
            with nogil:
                for i in prange(n):
                    result_array[i] = self._data[i] + other_data[i]
        else:
            # Sequential for small arrays
            for i in range(n):
                result_array[i] = self._data[i] + other_data[i]
        
        return self._create_result_array(list(result_array))
    
    def __sub__(self, other):
        """Element-wise subtraction with parallel processing."""
        cdef list other_data = self._check_compatible_shape(other)
        cdef Py_ssize_t n = len(self._data)
        cdef double[::1] result_array = np.empty(n, dtype=np.float64)
        cdef Py_ssize_t i
    ...
    def __mul__(self, other):
        ...
        for i in range(len(self._data)):
            result_data.append(self._data[i] * other_data[i])
    ...
    # Fast mathematical operations using C math functions with parallel processing
    def sqrt_fast(self):
        """Element-wise square root using C math with parallel processing."""
        cdef Py_ssize_t n = len(self._data)
        cdef double[::1] result_array = np.empty(n, dtype=np.float64)
        cdef double x
        cdef Py_ssize_t i
        
        # Use parallel processing for arrays larger than 500 elements
        if n > 500:
            with nogil:
                for i in prange(n):
                    result_array[i] = sqrt(self._data[i])
        else:
            # Sequential for small arrays
            for i in range(n):
                result_array[i] = sqrt(self._data[i])
    ...
    def sin_fast(self):
        """Element-wise sine using C math with parallel processing."""
        cdef Py_ssize_t n = len(self._data)
        cdef double[::1] result_array = np.empty(n, dtype=np.float64)
        cdef Py_ssize_t i
        ...
        if n > 500:
            with nogil:
                for i in prange(n):
                    result_array[i] = sin(self._data[i])
        else:
            # Sequential for small arrays
            for i in range(n):
                result_array[i] = sin(self._data[i])
    ...
    def cos_fast(self):
        """Element-wise cosine using C math with parallel processing."""
        cdef Py_ssize_t n = len(self._data)
        cdef double[::1] result_array = np.empty(n, dtype=np.float64)
        cdef Py_ssize_t i
        ...
        if n > 500:
            with nogil:
                for i in prange(n):
                    result_array[i] = cos(self._data[i])
        else:
            # Sequential for small arrays
            for i in range(n):
                result_array[i] = cos(self._data[i])
    ...
    def exp_fast(self):
        """Element-wise exponential using C math with parallel processing."""
        cdef Py_ssize_t n = len(self._data)
        cdef double[::1] result_array = np.empty(n, dtype=np.float64)
        cdef Py_ssize_t i
        ...
        if n > 500:
            with nogil:
                for i in prange(n):
                    result_array[i] = exp(self._data[i])
        else:
            # Sequential for small arrays
            for i in range(n):
                result_array[i] = exp(self._data[i])
    ...
    def log_fast(self):
        """Element-wise natural logarithm using C math with parallel processing."""
        cdef Py_ssize_t n = len(self._data)
        cdef double[::1] result_array = np.empty(n, dtype=np.float64)
        cdef Py_ssize_t i
        ...
        if n > 500:
            with nogil:
                for i in prange(n):
                    result_array[i] = log(self._data[i])
        else:
            # Sequential for small arrays
            for i in range(n):
                result_array[i] = log(self._data[i])
    ...
    @cython.wraparound(False)
    def sum_fast(self):
        """Calculate sum of array elements using fast iteration with parallel processing."""
    ...
    @cython.wraparound(False)
    def mean_fast(self):
        """Calculate arithmetic mean using fast iteration with parallel processing."""
        if not self._data:
    ...
    @cython.wraparound(True)  # Allow negative indexing for this method
    def percentile(self, q):
        ...
        sorted_data = sorted(self._data)
        cdef int n = len(sorted_data)
        
        if q == 0:
            return sorted_data[0]
        if q == 100:
            return sorted_data[n - 1]  # Use explicit indexing instead of -1
        
        cdef double index = (n - 1) * q / 100.0
        cdef int lower = int(index)
        cdef int upper = lower + 1
        
        if upper >= n:
            return sorted_data[lower]
        
        cdef double weight = index - lower
